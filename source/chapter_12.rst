...
事实上在ngx_http_core_content_phase中，对NGX_EAGAIN的返回会进入ngx_http_finalize_request调用，在该函数中对于r->buffered || c->buffered || r->postponed的request会执行ngx_http_set_write_handler，重新设置r->write_event_handler、r->read_event_handler，并加入epoll等。
